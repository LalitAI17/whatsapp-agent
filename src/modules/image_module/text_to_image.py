...
class ChatMessage(BaseModel):
    """Class representing a chat message that can be used with text_to_image.create_scenario."""
...
async def example_usage():
    ...
    # Example 1: Direct image generation with a simple prompt
    simple_prompt = "A serene mountain lake at sunset"
    enhanced_prompt = await text_to_image.enhance_prompt(simple_prompt)
    print(enhanced_prompt)
    image_data = await text_to_image.generate_image(
        enhanced_prompt, 
        output_path="./generated_images/lake_sunset.png"
    )
    print("done")
    
    # Create properly structured chat history
    chat_history = [
        ChatMessage(type="user", content="Tell me about ancient Egypt"),
        ChatMessage(type="assistant", content="Ancient Egypt was one of the world's first great civilizations, emerging along the Nile River around 3100 BCE. It was known for its monumental architecture like the pyramids and sphinx, hieroglyphic writing system, and complex religious beliefs centered around gods and the afterlife."),
        ChatMessage(type="user", content="What did the Great Pyramid of Giza look like in its prime?")
    ]
    
    # Generate the scenario
    scenario = await text_to_image.create_scenario(chat_history)
    ...
    image_data = await text_to_image.generate_image(
        ...
        output_path="./generated_images/pyramid.png"
    )
